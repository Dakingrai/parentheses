...
def process_comp(model, comp_activations, results, correct_idx, paren_token_ids):
    with torch.no_grad():
        ...
        softmax_projection = F.softmax(logit_projection, dim=-1)  # Shape: (batch, seq_len, vocab_size)
    
    paren_logits = [logit_projection[0, -1, i].item() for i in paren_token_ids]
    paren_softmax = [softmax_projection[0, -1, i].item() for i in paren_token_ids]

    # Compute entropy
    results["full_softmax_entropy"] = logit_entropy(softmax_projection[0, -1, :]).item()
    results["paren_entropy"] = logit_entropy(torch.tensor(paren_softmax)).item()
    ...
    results["paren_softmax"] = {
        "max-softmax": softmax_projection[0, -1, :].max().item(),
        "1-paren-softmax": softmax_projection[0, -1, paren_token_ids[0]].item(),
        "2-paren-softmax": softmax_projection[0, -1, paren_token_ids[1]].item(),
        "3-paren-softmax": softmax_projection[0, -1, paren_token_ids[2]].item(),
        "4-paren-softmax": softmax_projection[0, -1, paren_token_ids[3]].item()
    }
